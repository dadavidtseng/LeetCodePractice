...
public:
    bool canFinish(int numCourses, vector<vector<int>>& prerequisites) {
        ...
        // because the prerequisites.size() might be greater or equal than
        // numCourses
    ...
    bool dfs(vector<vector<int>> const& node, vector<int>& state, int i) {
        ...
        // Iterate through this course(node)'s every prerequisites(neighbor
        // nodes) return true if any cycle was found
        ...
        // Mark this course(node)'s state as completed since we didn't find any
        // cycle
        state[i] = 2;
