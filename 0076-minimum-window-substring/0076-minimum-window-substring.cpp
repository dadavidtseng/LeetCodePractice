...
public:
    string minWindow(string s, string t) {
        ...
        for (int i = 0; i < static_cast<int>(s.length()); ++i) {
            ...
            // if current char is in tWindow and is within that char count in
            // tWindow, and match +1
            if (tWindow[c] > 0 && window[c] == tWindow[c]) {
                matched++;
            }

            // while match == tWindow.size(), remove s[left] from window, and
            // move left +1
            while (matched == tDistinct) {
                // update minStart and minLength if windowLength is smaller than
                // minLength
