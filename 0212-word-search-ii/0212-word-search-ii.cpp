...
class Solution {
public:
    static vector<string> findWords(vector<vector<char>>& board,
                                    vector<string>& words) {
    ...
    static void backtrack(vector<vector<char>>& board, int const row,
                          int const col, Trie* trie, string& path,
                          vector<string>& result) {
        ...
        if (trie->children[idx] != nullptr) {
            ...
            // and mark this trie node as false so that it can be deleted in
            // next recursive call
            if (child->isEnd) {
            ...
            // temporarily marks the current cell's character before we go
            // deeper, so that we don't check the duplicated cell
            char const temp = c;
            c = '#';
            ...
            // this is pruning the recursive call for finding duplicated words
            // by deleting the trie node
