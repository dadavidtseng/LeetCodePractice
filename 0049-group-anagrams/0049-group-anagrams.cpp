...
public:
    //-----------------------------------------------------------------------------------------------
    // Custom hash functor for array<int, 26>, enabling its use as an
    // unordered_map key. Combines element hashes using the Boost hash_combine
    // technique.
    struct ArrayHash {
        size_t operator()(array<int, 26> const& arr) const {
            ...
            for (int val : arr) {
                seed ^=
                    hash<int>{}(val) + 0x9e3779b9 + (seed << 6) + (seed >> 2);
            }
    ...
    vector<vector<string>> groupAnagrams(vector<string>& strs) {
        ...
        for (unordered_map<array<int, 26>, vector<string>>::iterator it =
                 m.begin();
             it != m.end(); ++it) {
